...
router.patch("/:roleid", guildPermissionsMiddleware("MANAGE_ROLES"), rateLimitMiddleware(100, 1000 * 60 * 60), async (req, res) => {
    try {
        ...
        if (attempt) {
            ...
            dispatcher.dispatchEventToAllPerms(req.params.guildid, null, "MANAGE_ROLES", "GUILD_ROLE_UPDATE", {
                guild_id: req.params.guildid,
                role: role
            })
...
router.post("/", guildPermissionsMiddleware("MANAGE_ROLES"), rateLimitMiddleware(100, 1000 * 60 * 60), async (req, res) => {
    try {
        ...
        dispatcher.dispatchEventToAllPerms(req.params.guildid, null, "MANAGE_ROLES", "GUILD_ROLE_CREATE", {
            guild_id: req.params.guildid,
            role: role
        });
